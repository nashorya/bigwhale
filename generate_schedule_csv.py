...
def call_llm(prompt, system, cfg, image_path=None):
    """通过 OpenAI 兼容协议调用聊天模型，可选传入图片。"""
    m, k, u = cfg.get("model"), cfg.get("api_key"), cfg.get("base_url")

    if _OAI is None:
        raise ImportError("pip install openai")
    if not k or not m:
        raise ValueError("必须配置 API_KEY 和 CHAT_MODEL")

    client_kwargs = {"api_key": k}
    if u:
        client_kwargs["base_url"] = u
    client = _OAI(**client_kwargs)
    ...
    try:
        ...
        raise RuntimeError(f"API 网络请求失败或被兼容接口拒绝: {e}") from e

    if isinstance(response, str):
        raise ValueError(f"兼容接口返回了异常纯文本: {response}")
    if not hasattr(response, "choices") or not response.choices:
        raise ValueError(f"兼容接口返回数据残缺: {response}")
...
def check_subject_confidence(
    subjects: list[str], school: str, cfg: dict
) -> dict[str, str]:
    prompt = (
...
def collect_syllabi(low_subjects: list[str]) -> dict[str, str]:
    ...
    for subj in low_subjects:
        print(f"\n📌 科目：{subj}")
        print(
            "   👉 方式1：输入大纲所在的【本地文件路径】(如 D:\\dagang.txt) 并回车读取"
        )
...
def image_rows_to_timetable(rows: list) -> dict:
    ...
    headers, slots, busy, FREE = (
        rows[0],
        [r[0] for r in rows[1:] if r],
        [],
        {"", "自习", "None", "-", "——", "none", "null"},
    )
    for r in rows[1:]:
...
def parse_xlsx_timetable(xlsx_path: str) -> dict:
    ...
    rows = [
        row
        for row in [
            [get(r, c) for c in range(1, ws.max_column + 1)]
...
def timetable_to_free_desc(timetable: dict, cfg: dict) -> str:
    busy_lines = "\n".join(
        f"  {b['day']} {b['slot']}：{b['course']}" for b in timetable.get("busy", [])
    )
...
def generate_all(cfg, info, out_dir):
    ...
    all_subjects = (
        [s.strip() for s in info["subject1"].split(",")] if info["subject1"] else []
    ) + [s.strip() for s in info["subject2"].split(",")]
    ...
    try:
        ...
        for subj, level in confidence.items():
            print(
                f"  {'✅' if str(level).lower() == 'high' else '⚠️ '} {subj}：{'把握充足' if str(level).lower() == 'high' else '把握不足，需大纲'}"
            )
    except Exception as e:
    ...
    if monthly_data:
        ...
        active, suspended, week_label, focus = _extract_current_subjects(
            monthly_data, info
        )
        if suspended:
...
def main():
    ...
    ap.add_argument("--model", "-m", default=os.getenv("CHAT_MODEL"))
    ap.add_argument("--api-key", "-k", default=os.getenv("API_KEY"))
    ap.add_argument("--base-url", default=os.getenv("BASE_URL"))

    ap.add_argument(
        "--timetable", "-t", default=None, help="课表图片(jpg/png)或xlsx文件路径"
    )
    ap.add_argument(
        "--timetable-months",
        default="3-6月",
        dest="timetable_months",
        help="课表对应月份，如'3-6月'",
    )
    ...
    if not args.api_key or not args.model:
        print("❌ 请配置 API_KEY、CHAT_MODEL，或使用 -k、-m 参数传入")
        return
